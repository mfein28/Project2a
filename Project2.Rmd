...
The OLS estimate for the number of crimes on listing prices produces a coefficient of -0.010, suggesting that increasing the number of crimes by 1 will decrease the listing price by 1 cent. This coefficient is statistically significantly different from 0. Additionally, this model yields an R^2^ value of 0.021, indicating that only 2.1% of the variation in price is captured in this regression specification. Moreover, the p-value is significantly smaller than the alpha level of 0.05, suggesting that the overall model is significant.  

In general, the simple linear regression model substantiates the conventional hypothesis that higher crime will lead to lower prices. However, the low R^2^ shows that there is still work that can be done to improve the model. Now that we've established the connection between crime and prices, it is also interesting to determine what type of crime affects prices the most. Below is the regression output of price vs. type of crime:
